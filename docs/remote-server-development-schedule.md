...
**验收标准**:
- [x] 创建项目时可选择 local/remote 模式
- [x] ProjectModeDialog 组件正确显示和交互
- [x] Remote 模式下可选择远程服务器
- [x] ServiceFactory 支持 local/remote 模式切换
- [x] 模式切换无需重启应用
...
**验收标准**:
- [x] WebSocket 消息数量减少 > 50%
- [x] 延迟 < 100ms（聚合不影响用户体验）
- [x] 数据正确聚合,50ms 内的输出批量发送
- [x] 大数据块 (> 4KB) 立即刷新
- [x] PTY 退出时所有数据都被推送
...
**验收标准**:
- [x] 服务端启用 perMessageDeflate 压缩
- [x] 客户端启用 perMessageDeflate 压缩
- [x] 压缩可协商,不影响不支持压缩的客户端
- [x] 带宽占用预期减少 > 30%（文本输出）
...
**验收标准**:
- [x] 断线后自动重连,使用指数退避策略(1s→30s)
- [x] 重连成功后会话自动恢复
- [x] UI 显示重连状态(Toast 提示)
- [x] 主动关闭的连接不触发重连
- [x] 用户感知延迟 < 5 秒
...
**目标**: 完善部署方案、文档、测试（按实用主义优先级）
**周期**: 2-3 周
**关键产出**: 可生产部署的完整系统

**设计原则**:
- 优先解决最实际的问题（Docker 部署）
- 文档跟着功能走，不过度设计
- 测试持续进行，不堆到最后
- **简单优于复杂，质量优于数量**

**调整说明**:
- ✅ 优先级 1-2 已完成（Docker 部署 + 核心文档）
- ❌ 优先级 3 已跳过（Systemd + Nginx，文档中已有配置说明）
- ⏭️ 优先级 4 可选执行（手动测试优于自动化测试）

---

### 优先级 1: Docker 部署（最实用，必须先做）
...
- [x] 数据持久化（重启后数据不丢失）
- [x] 自动生成 AUTH_SECRET 并保存到 .env 文件
- [x] 健康检查正常工作
...
- [x] 所有 API 有文档覆盖
- [x] 示例代码可运行
- [x] 包含 gRPC API (5个服务) 和 WebSocket API
- [x] 包含认证说明、错误处理、完整示例
...
**验收标准**:
- [x] 涵盖所有部署方式（Docker Compose、Systemd、手动）
- [x] 安全最佳实践完整（TLS、防火墙、SSH 加固）
- [x] 包含监控日志、备份恢复、升级流程
- [x] 提供 Nginx 反向代理配置
- [x] 生产部署检查清单
...
### 优先级 3: 其他部署方式 ~~（已跳过 - 不必要）~~

**决策**: 不执行此优先级的任务

**理由**:
1. **Docker Compose 已经是最简单的部署方式**，覆盖 95% 的使用场景
2. **deployment.md 已包含完整的 Systemd 和 Nginx 配置说明**，有需要的用户可以自行配置
3. **避免增加维护负担**：多一种部署方式 = 多一份技术债
4. **时间应该花在测试和验证上**，而不是增加可选功能

**替代方案**:
- Systemd 用户：参考 `docs/deployment.md` 中的完整配置指南
- Nginx 用户：参考 `docs/deployment.md` 中的反向代理配置示例

**详细决策说明**: 参考 [`server/docs/phase3-priority3-skip-decision.md`](../server/docs/phase3-priority3-skip-decision.md)

---

#### ~~任务 8.3: Systemd Service~~（跳过）
**工时**: 1 天
**负责**: DevOps
**依赖**: Phase 2 完成

**状态**: ❌ 跳过 - 已在 deployment.md 中提供完整配置说明
...
**注**: 用户可参考 `deployment.md` 中的详细步骤自行配置

---

#### ~~任务 8.4: Nginx 反向代理配置~~（跳过）
**工时**: 1 天
**负责**: DevOps
**依赖**: 任务 7.2

**状态**: ❌ 跳过 - 已在 deployment.md 中提供完整配置示例
...
**注**: 用户可参考 `deployment.md` 中的 Nginx 配置示例自行部署
...
### Milestone 3: 生产就绪（调整后）
**交付物**:
- ✅ Docker 镜像 + Compose 配置（已完成）
- ❌ ~~Systemd Service~~（跳过 - 已在文档中提供配置说明）
- ✅ 完整文档（用户指南、API、部署）（已完成）
- ⏭️ 测试报告（E2E、性能、安全）（手动测试可选）
- ⏭️ 已知问题列表（待测试后记录）
...
**上线标准**:
- [ ] 所有测试通过（优先级 4 待完成）
- [x] 文档完整（用户指南、API 文档、部署文档 ✅）
- [x] 部署脚本可用（Docker Compose + 启动脚本 ✅）
- [ ] 无 P0/P1 级 Bug（需要测试验证）

**当前进度**: 优先级 1-2 已完成，优先级 3-4 待完成
...
本开发排期最初规划了 **3 个阶段、10 周**，实际执行中根据实用主义原则进行了调整。

**已完成阶段：**
- ✅ **Phase 1: 基础架构**（Week 1-3）- 客户端抽象层、Golang 服务端、远程 PTY
- ✅ **Phase 2: 完整功能**（Week 4-7）- 远程 Agent、配置管理、性能优化、安全认证
- ✅ **Phase 3 优先级 1-2**（Week 8-9）- Docker 部署、核心文档

**跳过的任务：**
- ❌ **Phase 3 优先级 3**（Systemd Service + Nginx 反向代理）
  - **理由**: Docker Compose 已经是最简单的方案，文档中已包含完整配置说明
  - **替代**: `docs/deployment.md` 提供详细的 Systemd 和 Nginx 配置指南

**待选执行：**
- ⏭️ **Phase 3 优先级 4**（测试与修复）
  - 建议：手动测试优于自动化测试（项目早期阶段）
  - 重点：端到端功能验证、性能基准测试、安全检查
...
5. **实用主义**: 简单优于复杂，质量优于数量 ✅

### 实用主义调整

基于 Linus Torvalds 的工程哲学：
...
3. **"解决真实问题，而非假想问题"**
   ...
   - 极少数需要 Systemd 的用户可以自行配置

### 下一步行动

1. **Milestone 3 验收**: 实际部署测试
   - 部署到测试服务器
   - 客户端连接测试
   - 端到端功能验证
   - 记录发现的问题
